...
coldata <- coldata %>% filter(Tissue == "LEGS")
countdata <- countdata[,rownames(coldata)]
...
de_legs <- NULL
de_legs <- voom(dge, design, plot=TRUE)
model_legs <- duplicateCorrelation(de_legs, design, block = coldata$ID)
# this will include the block variable (ID or pairings) as well as corr estimates
de_legs <- voom(dge,design,plot=TRUE,block=coldata$ID,
                  correlation=model_legs$consensus)
model_legs <- duplicateCorrelation(de_legs, design, block = coldata$ID)


fit_legs <- lmFit(de_legs, design, block=coldata$ID, 
                    correlation=model_legs$consensus)
fit_legs <- eBayes(fit_legs, robust = TRUE)

de_results_legs <- topTable(fit_legs, coef=2, sort.by = "p", n =Inf)

tmp <- gsub("\\..*","",row.names(de_results_legs))
de_results_legs$symbol <- mapIds(org.Hs.eg.db, keys=tmp, column="SYMBOL", keytype="ENSEMBL", multiVals="first") 
de_results_legs$Entrez <- mapIds(org.Hs.eg.db, keys=tmp, column="ENTREZID", keytype="ENSEMBL", multiVals="first") 
de_results_legs <- na.omit(de_results_legs)
de_results_legs <- de_results_legs[order(de_results_legs$adj.P.Val),]
de_results_legs$Abs.logFC <- abs(de_results_legs$logFC)
de_sig_legs <- de_results_legs %>% filter(adj.P.Val <= 0.1)

write.csv(de_results_legs,"results/de_legs.csv")


# de_genes <- de_results_legs %>% dplyr::select(symbol,logFC,adj.P.Val)
