...
design <- model.matrix(~coldata$Tissue+coldata$State)
...
de_subset <- NULL
de_subset <- voom(dge, design, plot=TRUE)
model_subset <- duplicateCorrelation(de_subset, design, block = coldata$ID)
# this will include the block variable (ID or pairings) as well as corr estimates
de_subset <- voom(dge,design,plot=TRUE,block=coldata$ID,
                  correlation=model_subset$consensus)
model_subset <- duplicateCorrelation(de_subset, design, block = coldata$ID)


fit_subset <- lmFit(de_subset, design, block=coldata$ID, 
                    correlation=model_subset$consensus)
fit_subset <- eBayes(fit_subset, robust = TRUE)

de_results_subset <- topTable(fit_subset, coef=2, sort.by = "p", n =Inf)

tmp <- gsub("\\..*","",row.names(de_results_subset))
de_results_subset$symbol <- mapIds(org.Hs.eg.db, keys=tmp, column="SYMBOL", keytype="ENSEMBL", multiVals="first") 
de_results_subset$Entrez <- mapIds(org.Hs.eg.db, keys=tmp, column="ENTREZID", keytype="ENSEMBL", multiVals="first") 
de_results_subset <- na.omit(de_results_subset)
de_results_subset <- de_results_subset[order(de_results_subset$adj.P.Val),]
de_results_subset$Abs.logFC <- abs(de_results_subset$logFC)
de_sig_subset <- de_results_subset %>% filter(adj.P.Val <= 0.1)

write.csv(de_results_subset,"results/de_subset.csv")


# de_genes <- de_results_subset %>% dplyr::select(symbol,logFC,adj.P.Val)
