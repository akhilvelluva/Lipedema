...
# Only Lipedema Arms where Belly is Controls ------------------------------
...
coldata <- coldata %>% filter(Tissue %in% c("ARMS","BELLY"),State == "LIPEDEMA")
...
ggsave("results/PCA_case_arms.png", )
...
de_case_arms <- NULL
de_case_arms <- voom(dge, design, plot=TRUE)
model_case_arms <- duplicateCorrelation(de_case_arms, design, block = coldata$ID)
# this will include the block variable (ID or pairings) as well as corr estimates
de_case_arms <- voom(dge,design,plot=TRUE,block=coldata$ID,
                correlation=model_case_arms$consensus)
model_case_arms <- duplicateCorrelation(de_case_arms, design, block = coldata$ID)


fit_case_arms <- lmFit(de_case_arms, design, block=coldata$ID, 
                  correlation=model_case_arms$consensus)
fit_case_arms <- eBayes(fit_case_arms, robust = TRUE)

de_results_case_arms <- topTable(fit_case_arms, coef=2, sort.by = "p", n =Inf)

tmp <- gsub("\\..*","",row.names(de_results_case_arms))
de_results_case_arms$symbol <- mapIds(org.Hs.eg.db, keys=tmp, column="SYMBOL", keytype="ENSEMBL", multiVals="first") 
de_results_case_arms$Entrez <- mapIds(org.Hs.eg.db, keys=tmp, column="ENTREZID", keytype="ENSEMBL", multiVals="first") 
de_results_case_arms <- na.omit(de_results_case_arms)
de_results_case_arms <- de_results_case_arms[order(de_results_case_arms$adj.P.Val),]
de_results_case_arms$Abs.logFC <- abs(de_results_case_arms$logFC)
de_sig_case_arms <- de_results_case_arms %>% filter(adj.P.Val <= 0.1)

write.csv(de_results_case_arms,"results/de_case_arms.csv")


# de_genes <- de_results_case_arms %>% dplyr::select(symbol,logFC,adj.P.Val)
